...
[![Maintained by next-linux.systems](https://img.shields.io/badge/maintained%20by-next-linux.systems-blueviolet)](https://next-linux.systems/?utm_source=github&utm_medium=organic_oss&utm_campaign=harden-runner)
[![OpenSSF Scorecard](https://api.securityscorecards.dev/projects/github.com/nextlinux/harden-runner/badge)](https://api.securityscorecards.dev/projects/github.com/nextlinux/harden-runner)
...
| --- | -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1.  | Block outbound calls that are not in the allowed list to prevent exfiltration of credentials | To prevent [Codecov breach](https://github.com/nextlinux/attack-simulator/blob/main/docs/RestrictOutboundTraffic.md) scenario                                                                                                         |
| 2.  | Detect if source code is being overwritten during the build process to inject a backdoor     | To detect [SolarWinds incident scenario](https://github.com/nextlinux/attack-simulator/blob/main/docs/MonitorSourceCode.md)                                                                                                           |
| 3.  | Detect compromised dependencies that make unexpected outbound network calls                  | To detect [Dependency confusion](https://github.com/nextlinux/attack-simulator/blob/main/docs/DNSExfiltration.md) and [Malicious dependencies](https://github.com/nextlinux/attack-simulator/blob/main/docs/CompromisedDependency.md) |
...
1. Add `nextlinux/harden-runner` to your GitHub Actions workflow file as the first step in each job.

   ```yaml
   steps:
     - uses: nextlinux/harden-runner@128a63446a954579617e875aaab7d2978154e969 # v2.4.0
       with:
         egress-policy: audit
   ```
...
For details, check out the documentation at https://docs.next-linux.systems
...
Once allowed endpoints are set in the policy in the workflow file, or in the [Policy Store](https://docs.next-linux.systems/harden-runner/how-tos/block-egress-traffic#2-add-the-policy-using-the-policy-store)
...
Private repositories are supported if they have a commercial license. Check out the [documentation](https://docs.next-linux.systems/harden-runner/installation/business-enterprise-license) for more details.
...
Read this [case study on how Kapiche uses Harden Runner](https://www.next-linux.systems/case-studies/kapiche/) to improve software supply chain security in their open source and private repositories.

## Discussions

If you have questions or ideas, please use [discussions](https://github.com/nextlinux/harden-runner/discussions).

1. [Support for private repositories](https://github.com/nextlinux/harden-runner/discussions/74)
2. [Where should allowed-endpoints be stored?](https://github.com/nextlinux/harden-runner/discussions/84)
3. [Cryptographically verify tools run as part of the CI/ CD pipeline](https://github.com/nextlinux/harden-runner/discussions/94)

## Limitations

1. Harden-Runner GitHub Action only works for GitHub-hosted runners. Self-hosted runners are not supported. We have started work on supporting [Kubernetes-Based Self-Hosted Actions Runners](https://github.com/nextlinux/harden-runner/issues/104).
2. Only Ubuntu VM is supported. Windows and MacOS GitHub-hosted runners are not supported. There is a discussion about that [here](https://github.com/nextlinux/harden-runner/discussions/121).
...
|--|----------|----------------|
|1.|[nvm-sh/nvm](https://github.com/nvm-sh/nvm/blob/master/.github/workflows/lint.yml)|[Link to insights](https://app.next-linux.systems/github/nvm-sh/nvm/actions/runs/1757959262)|
|2.|[jsx-eslint/eslint-plugin-react](https://github.com/jsx-eslint/eslint-plugin-react/blob/master/.github/workflows/release.yml)|[Link to insights](https://app.next-linux.systems/github/yannickcr/eslint-plugin-react/actions/runs/1930818585)
|3.|[microsoft/msquic](https://github.com/microsoft/msquic/blob/main/.github/workflows/docker-publish.yml)|[Link to insights](https://app.next-linux.systems/github/microsoft/msquic/actions/runs/1759010243)
|4.|[ossf/scorecard](https://github.com/ossf/scorecard/blob/main/.github/workflows/codeql-analysis.yml)|[Link to insights](https://app.next-linux.systems/github/ossf/scorecard/actions/runs/2006162141)
|5.|[Automattic/vip-go-mu-plugins](https://github.com/Automattic/vip-go-mu-plugins/blob/master/.github/workflows/e2e.yml)|[Link to insights](https://app.next-linux.systems/github/Automattic/vip-go-mu-plugins/actions/runs/1758760957)
...
- The agent is written in Go and is open source at https://github.com/nextlinux/agent
- The agent's build is reproducible. You can view the steps to reproduce the build [here](http://app.next-linux.systems/github/nextlinux/agent/releases/latest)
